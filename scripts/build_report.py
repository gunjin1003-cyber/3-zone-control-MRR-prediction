...
MODELS = [
    ...
    ("반경별 능형회귀 [quad]", 163.6, "lin"),
    ("PCA(4) + 능형회귀 [quad]", 163.8, "lin"),
...
{bars}
  ...
  <div class="col">
    ...
    <ul>
      ...
      <li><strong>상위 4종은 서로 구분되지 않습니다.</strong> 같은 웨이퍼를 예측하므로
        오차가 강하게 상관되어 있어, fold 단위로 짝지어 다시 재면 훨씬 민감해집니다.
        그렇게 재도 대칭성 능형회귀가 Quad Ridge를 이기는 폭이 <em>0.6 A/min</em>
        (16 fold 중 12승) — 통계적으로는 검출되지만 재현성이 118인 공정에서는 없는 차이입니다.
        1차만 쓴 모델(169.2)조차 신뢰구간이 0을 걸칩니다.</li>
      <li><strong>앙상블도 도움이 되지 않습니다.</strong> 상위 4종의 모든 조합을 평균내
        보았지만 최고가 162.9 — 단일 모델 대비 0.3 개선으로, SE 24 앞에서는 노이즈입니다.
        상위 모델들이 이미 노이즈 하한 근처에서 <em>같은 오차</em>를 내고 있어
        평균내도 상쇄될 것이 없습니다.</li>
    </ul>
    <p>하이퍼파라미터 탐색까지 fold 안에 넣은 중첩 교차검증으로 다시 재도 순위는 같습니다 —
      선형·저차원 모델이 168~178 로 상위권을 독식하고, 트리 계열 3종이 218~232 로 나란히
      최하위입니다. 트리가 꼴찌인 것은 우연이 아니라 아래 07의 외삽 실패와 같은 원인입니다.</p>
  </div>
</section>
